...
@asset(
    name="ts_green_trips", key_prefix=["ts", "green_trips"],
    description="download and process green_trips data",
    retry_policy=retry_policy,
    compute_kind="duckdb", deps=[ts_dim_table]
)
def ts_green_trips(context: AssetExecutionContext, duckdb: DuckDBResource):
    context.log.info("API List")
    context.log.info(API_LIST_GREEN)
    ...
    with duckdb.get_connection() as conn:
        for idx, sub_list in enumerate(sub_lists):
            ...
            create_table_from_polars_dataframe(df, TRIPS_SCHEMA, TABLE_TS_GREEN_TRIPS, mapping_polars_dtype,
                                               conn, context)
            view_raw_data(data, TRIPS_SCHEMA, TABLE_TS_GREEN_TRIPS, context)

            insert_data_into_table(conn, data, TRIPS_SCHEMA, TABLE_TS_GREEN_TRIPS, context)

            process_static_table(conn=conn, fact_schema=TRIPS_SCHEMA, fact_table=TABLE_TS_GREEN_TRIPS,
            ...
            process_static_table(conn=conn, fact_schema=TRIPS_SCHEMA, fact_table=TABLE_TS_GREEN_TRIPS,
                                 dim_schema=TRIPS_SCHEMA, dim_table=TABLE_TS_DIM_TRIP_TYPE,
                                 dim_column="trip_type", dim_table_id_column="id", context=context)

            process_static_table(conn=conn, fact_schema=TRIPS_SCHEMA, fact_table=TABLE_TS_GREEN_TRIPS,
                                 dim_schema=TRIPS_SCHEMA, dim_table=TABLE_TS_DIM_VENDOR,
                                 dim_column="vendorid", dim_table_id_column="id", context=context)

            create_secret(conn=conn, secret_name=SECRET_TS_GREEN_TRIPS, secret_type=SECRET_TYPE,
                          access_key_id=ACCESS_KEY, secret_access_key=SECRET_KEY, context=context)

            copy_file_to_s3(conn=conn, bucket_name=MD_BUCKET,
                            schema=TRIPS_SCHEMA, table=TABLE_TS_GREEN_TRIPS,
                            file_name=TABLE_TS_GREEN_TRIPS.lower().replace("_", ""), context=context, idx=idx)
            ...
            copy_file_to_s3(conn=conn, bucket_name=MD_BUCKET,
                            schema=TRIPS_SCHEMA, table=TABLE_TS_DIM_TRIP_TYPE,
                            file_name=TABLE_TS_DIM_TRIP_TYPE.lower().replace("_", ""), context=context, idx=idx)
